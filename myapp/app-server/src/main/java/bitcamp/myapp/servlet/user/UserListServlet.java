...
public class UserListServlet implements Servlet {
  ...
  public ServletConfig getServletConfig() {
    ...
    // 따라서 init() 메서드가 호출될 때 ServletConfig 객체를 보관해 두어야 한다.
    return this.config;
  }
  ...
  public void service(ServletRequest req, ServletResponse res)
    ...
    try {
      ...
      for (User user : userDao.list()) {
        out.printf("<tr><td>%d</td><td><a href=%s>%s</a></td><td>%s</td></tr>\n",
            "/user/list?" + user.getNo(), user.getNo(), user.getName(),
            user.getEmail());
      }
      out.println("</tbody>");
      out.println("</table>");
  ...
  public String getServletInfo() {
    // 서블릿 컨테이너 관리 화면에서 서블릿 정보를 출력할 때 이 메서드가 호출된다.
