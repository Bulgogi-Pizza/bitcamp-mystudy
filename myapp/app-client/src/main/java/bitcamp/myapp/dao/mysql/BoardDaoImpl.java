...
public class BoardDaoImpl implements BoardDao {
  ...
  public boolean insert(Board board) throws Exception {
    sqlSession.insert("insert into myapp_boards(title, content, user_id) values (?, ?, ?)",
        board.getTitle(),
        board.getContent(),
  ...
  public Board findBy(int no) throws Exception {
    return sqlSession.selectOne("select "
            + " b.board_id,"
            + " b.title,"
            + " b.content,"
            + " b.created_date,"
  ...
  public boolean update(Board board) throws Exception {
    int count = sqlSession.update("update myapp_boards set"
            + " title=?,"
            + " content=?"
            + " where board_id=?",
        board.getTitle(),
